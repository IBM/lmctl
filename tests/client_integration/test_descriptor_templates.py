...
class TestDescriptorTemplatesAPI(ClientIntegrationTest):

    def test_all(self):
        templates_api = self.tester.default_client.descriptor_templates
        descriptor_template_A = self.tester.load_descriptor_from(self.tester.test_file('dummy_template.yaml'))
        name_part = self.tester.exec_prepended_name('test-all-A')
        descriptor_template_A['name'] = f'assembly-template::{name_part}::1.0'
        descriptor_template_B = self.tester.load_descriptor_from(self.tester.test_file('dummy_template.yaml'))
    ...
    def test_crud(self):
        descriptor_template = self.tester.load_descriptor_from(self.tester.test_file('dummy_template.yaml'))
        templates_api = self.tester.default_client.descriptor_templates
        ## Create
