...
class TestResourceManagersAPI(ClientIntegrationTest):
    ...
    def before_test_case(cls, tester):
        ...
        tester.build_resource_package_from(tester.test_file('dummy_resource'), res_pkg_path)
        cls.test_case_props['dummy_res_pkg_id'] = tester.default_client.resource_packages.create(res_pkg_path)
        resource_descriptor = tester.load_descriptor_from(tester.test_file(os.path.join('dummy_resource', 'Definitions', 'lm', 'resource.yaml')))
