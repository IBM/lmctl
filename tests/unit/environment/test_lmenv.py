...
class TestTNCOEnvironment(unittest.TestCase):
    ...
    def test_minimum_init(self):
        config = TNCOEnvironment('Example', address='testing')
    ...
    def test_init_fails_when_name_is_none(self):
        with self.assertRaises(ValidationError) as context:
            config = TNCOEnvironment(None, address='localhost')
    ...
    def test_init_fails_when_address_and_host_are_none(self):
        with self.assertRaises(ValidationError) as context:
            config = TNCOEnvironment('Example')
    ...
    def test_init_fails_when_secure_and_credentials_not_set(self):
        with self.assertRaises(ValidationError) as context:
            config = TNCOEnvironment('lm', host='test', port=80, secure=True)
    ...
    def test_init_defaults_port_and_protocol_and_path(self):
        config = TNCOEnvironment('lm', host='test')
        self.assertEqual(config.address, 'https://test')

    def test_init_auth_defaults_auth_address(self):
        config = TNCOEnvironment('lm', host='test', port=80, secure=True, username='test')
    ...
    def test_init_with_password(self):
        config = TNCOEnvironment('lm', host='test', port=80, secure=True, username='test', password='secret')
        self.assertEqual(config.username, 'test')
        self.assertEqual(config.password, 'secret')

    def test_init_with_all_as_parts(self):
        config = TNCOEnvironment('lm', host='test', port=80, protocol='http', path='gateway', secure=True, username='user', password='secret', auth_host='auth', auth_port=81, auth_protocol='https', brent_name='alt_brent')
    ...
    def test_init_with_address(self):
        config = TNCOEnvironment('lm', address='https://test:8080/api')
    ...
    def test_is_secure(self):
        insecure_env = TNCOEnvironment('lm', host='test', port=80)
        self.assertFalse(insecure_env.secure)
        secure_env = TNCOEnvironment('lm', host='test', port=80, secure=True, username='user')
        self.assertTrue(secure_env.secure)

    def test_address_from_parts(self):
        env = TNCOEnvironment('lm', host='test')
        self.assertEqual(env.address, 'https://test')
        env = TNCOEnvironment('lm', host='test', port=32455)
        self.assertEqual(env.address, 'https://test:32455')
        env = TNCOEnvironment('lm', host='test', port=32455, protocol='http')
        self.assertEqual(env.address, 'http://test:32455')
        env = TNCOEnvironment('lm', host='test', port=None, protocol='http')
        self.assertEqual(env.address, 'http://test')
        env = TNCOEnvironment('lm', host='test', port=None, protocol='http', path='gateway')
        self.assertEqual(env.address, 'http://test/gateway')
        env = TNCOEnvironment('lm', host='test', port=32455, protocol='http', path='gateway')
        self.assertEqual(env.address, 'http://test:32455/gateway')

    def test_auth_address_from_parts(self):
        env = TNCOEnvironment('lm', host='test', port=80, protocol='http', secure=True, username='user', auth_host='auth', auth_port=82, auth_protocol='https')
    ...
    def test_kami_address_override_port(self):
        env = TNCOEnvironment('lm', address='http://test:8080', kami_port=5678)
        self.assertEqual(env.kami_address, 'http://test:5678')

    def test_kami_address_override_protocol(self):
        env = TNCOEnvironment('lm', address='http://test:8080', kami_protocol='https')
        self.assertEqual(env.kami_address, 'https://test:31289')

    def test_zen_auth_mode(self):
        env = TNCOEnvironment('lm', address='http://test:8080', secure=True, auth_mode='zen', username='Zenny', api_key='12345', auth_address='http://zen:8000/api')
    ...
    def test_zen_auth_missing_username(self):
        with self.assertRaises(ValidationError) as context:
            TNCOEnvironment('lm', address='http://test:8080', secure=True, auth_mode='zen', auth_address='http://zen:8000/api')
    ...
    def test_zen_auth_missing_auth_address(self):
        with self.assertRaises(ValidationError) as context:
            TNCOEnvironment('lm', address='http://test:8080', secure=True, auth_mode='zen', username='user', api_key='API')
    ...
    def test_create_session_config_with_username_password(self):
        env = TNCOEnvironment('lm', host='test', port=80, protocol='https', secure=True, username='user')
        session_config = env.create_session_config()
        self.assertEqual(session_config.username, 'user')
        self.assertEqual(session_config.password, None)
        env = TNCOEnvironment('lm', host='test', port=80, protocol='https', secure=True, username='user', password='secret')
        session_config = env.create_session_config()
        self.assertEqual(session_config.username, 'user')
    ...
    def test_create_session_config_with_client_credentials(self):
        env = TNCOEnvironment('lm', host='test', port=80, protocol='https', secure=True, client_id='client', client_secret='secret')
        session_config = env.create_session_config()
    ...
    def test_create_session_config_with_zen_auth(self):
        env = TNCOEnvironment('lm', host='test', port=80, protocol='https', secure=True, username='user', api_key='key', auth_address='http://zen:80', auth_mode='zen')
        session_config = env.create_session_config()
        self.assertEqual(session_config.username, 'user')
...
class TestLmSession(unittest.TestCase):
    ...
    def test_security_ctrl_builds_client_with_legacy_auth(self, mock_client_builder_init):
        env = TNCOEnvironment('lm', address='http://api:80', secure=True, username='user', password='secret', auth_address='http://auth:81')
        session_config = env.create_session_config()
        session = LmSession(session_config)
    ...
    def test_security_ctrl_builds_client_with_legacy_auth_on_default_address(self, mock_client_builder_init):
        env = TNCOEnvironment('lm', address='http://api:80', secure=True, username='user', password='secret')
        session_config = env.create_session_config()
        session = LmSession(session_config)
        driver = session.descriptor_driver #Force LmSecurityCtrl to be created
        mock_client_builder = mock_client_builder_init.return_value 
    ...
    def test_security_ctrl_builds_client_with_user_pass_auth(self, mock_client_builder_init):
        env = TNCOEnvironment('lm', address='http://api:80', secure=True, username='user', password='secret', client_id='UserClient', client_secret='c_secret', auth_address='http://auth:81')
        session_config = env.create_session_config()
        session = LmSession(session_config)
        driver = session.descriptor_driver #Force LmSecurityCtrl to be created
        mock_client_builder = mock_client_builder_init.return_value 
    ...
    def test_security_ctrl_builds_client_with_user_pass_auth_on_default_address(self, mock_client_builder_init):
        env = TNCOEnvironment('lm', address='http://api:80', secure=True, username='user', password='secret', client_id='UserClient', client_secret='c_secret')
        session_config = env.create_session_config()
        session = LmSession(session_config)
        driver = session.descriptor_driver #Force LmSecurityCtrl to be created
        mock_client_builder = mock_client_builder_init.return_value 
    ...
    def test_security_ctrl_builds_client_with_client_auth(self, mock_client_builder_init):
        env = TNCOEnvironment('lm', address='http://api:80', secure=True, client_id='UserClient', client_secret='c_secret', auth_address='http://auth:81')
        session_config = env.create_session_config()
        session = LmSession(session_config)
        driver = session.descriptor_driver #Force LmSecurityCtrl to be created
        mock_client_builder = mock_client_builder_init.return_value 
    ...
    def test_security_ctrl_builds_client_with_client_auth_on_default_address(self, mock_client_builder_init):
        env = TNCOEnvironment('lm', address='http://api:80', secure=True, client_id='UserClient', client_secret='c_secret')
        session_config = env.create_session_config()
        session = LmSession(session_config)
        driver = session.descriptor_driver #Force LmSecurityCtrl to be created
        mock_client_builder = mock_client_builder_init.return_value 
    ...
    def test_descriptor_driver(self, descriptor_driver_init):
        session = LmSession(LmSessionConfig(TNCOEnvironment('lm', host='test', port=80, protocol='https'), None, auth_mode='oauth'))
    ...
    def test_onboard_rm_driver(self, onboard_rm_driver_init):
        session = LmSession(LmSessionConfig(TNCOEnvironment('lm', host='test', port=80, protocol='https'), None))
