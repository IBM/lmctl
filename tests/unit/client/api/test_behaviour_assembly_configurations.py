...
class TestBehaviourAssemblyConfigurationsAPI(unittest.TestCase):
    ...
    def test_create(self):
        ...
        self.mock_client.make_request.assert_called_with(TNCOClientRequest(method='POST', endpoint='api/behaviour/assemblyConfigurations', headers={'Content-Type': 'application/json'}, body={'name': 'Test'}))

    def test_update(self):
        test_obj = {'id': '123', 'name': 'Test'}
        response = self.behaviour_assembly_configurations.update(test_obj)
        self.assertIsNone(response)
        self.mock_client.make_request.assert_called_with(TNCOClientRequest(method='PUT', endpoint='api/behaviour/assemblyConfigurations/123', headers={'Content-Type': 'application/json'}, body={'id': '123', 'name': 'Test'}))
