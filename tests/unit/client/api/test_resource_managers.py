...
class TestResourceManagersAPI(unittest.TestCase):
    ...
    def test_all(self):
        ...
        self.mock_client.make_request.assert_called_with(TNCOClientRequest.build_request_for_json(method='GET', endpoint='api/v1/resource-managers'))
    ...
    def test_get(self):
        ...
        self.mock_client.make_request.assert_called_with(TNCOClientRequest.build_request_for_json(method='GET', endpoint='api/v1/resource-managers/Test'))

    def test_create(self):
        test_obj = {'name': 'Test'}
        body = json.dumps(test_obj)
        mock_response = MagicMock(headers={'Location': '/api/v1/resource-managers/123'})
        ...
        self.mock_client.make_request.assert_called_with(TNCOClientRequest(method='POST', endpoint='api/v1/resource-managers', headers={'Content-Type': 'application/json'}, body=body))
    ...
    def test_update(self):
        ...
        self.assertEqual(response, mock_onboarding_report)
        self.mock_client.make_request.assert_called_with(TNCOClientRequest(method='PUT', endpoint='api/v1/resource-managers/Test', headers={'Content-Type': 'application/json'}, body=body))

    def test_delete(self):
        response = self.resource_managers.delete('Test')
        self.assertIsNone(response)
        self.mock_client.make_request.assert_called_with(TNCOClientRequest(method='DELETE', endpoint='api/v1/resource-managers/Test'))
