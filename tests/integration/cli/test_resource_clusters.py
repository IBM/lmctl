...
class TestClusters(CLIIntegrationTest):
    ...
    def test_scale_with_assembly_name(self):
        ...
        scale_result = self.cli_runner.invoke(cli, [
            'scale', 'resourcecluster', '-e', 'default', 'B', '--assembly-name', assembly_name, '--out'
            ])
    ...
    def test_scale_with_assembly_id(self):
        ...
        scale_result = self.cli_runner.invoke(cli, [
            'scale', 'resourcecluster', '-e', 'default', 'B', '--assembly-id', assembly_id, '--out'
            ])
        self._confirm_process_success(scale_result)
        assembly = self.tester.default_client.assemblies.get_by_name(assembly_name)
        self.assertEqual(len(self._get_child(assembly, 'B')['instances']), 2)
    ...
    def test_scale_with_assembly_id_and_name_fails(self):
        scale_result = self.cli_runner.invoke(cli, [
            'scale', 'resourcecluster', '-e', 'default', 'B', '--assembly-name', 'SomeName', '--assembly-id', 'SomeId', '--out'
            ])
        self.assert_has_system_exit(scale_result)
        expected_output = 'Usage: cli scale resourcecluster [OPTIONS] [NAME]'
        expected_output += '\nTry \'cli scale resourcecluster --help\' for help.'
    ...
    def test_scale_without_assembly_id_or_name_fails(self):
        scale_result = self.cli_runner.invoke(cli, [
            'scale', 'resourcecluster', '-e', 'default', 'B', '--out'
            ])
        self.assert_has_system_exit(scale_result)
        expected_output = 'Usage: cli scale resourcecluster [OPTIONS] [NAME]'
        expected_output += '\nTry \'cli scale resourcecluster --help\' for help.'
        expected_output += '\n\nError: Must identify the target by specifying one parameter from ["--assembly-name", "--assembly-id"] or by including one of the following attributes ["assemblyName", "assemblyId"] in the given object/file'
        self.assert_output(scale_result, expected_output)
    ...
    def test_scale_in(self):
        ...
        self.assertEqual(len(self._get_child(assembly, 'B')['instances']), 2)
        scale_result = self.cli_runner.invoke(cli, [
            'scale', 'resourcecluster', '-e', 'default', 'B', '--assembly-name', assembly_name, '--in'
            ])
        self._confirm_process_success(scale_result)
        assembly = self.tester.default_client.assemblies.get_by_name(assembly_name)
    ...
    def test_scale_without_in_or_out_option_defaults_to_out(self):
        assembly_name = self.tester.exec_prepended_name('cluster-cmd-scale-default')
        self._create_assembly(assembly_name)
        scale_result = self.cli_runner.invoke(cli, [
            'scale', 'resourcecluster', '-e', 'default', 'B', '--assembly-name', assembly_name
            ])
        self._confirm_process_success(scale_result)
        assembly = self.tester.default_client.assemblies.get_by_name(assembly_name)
        self.assertEqual(len(self._get_child(assembly, 'B')['instances']), 2)
