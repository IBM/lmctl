...
class SitePlannerAPITests:
    ...
    class BaseTest(IntegrationTest):
        ...
        def _compare(self, sent_data, retrieved_data, is_update=False, provided_ignore_fields=None):
            if provided_ignore_fields is None:
                provided_ignore_fields = []
            for k,v in sent_data.items():
                if k not in provided_ignore_fields and k not in self._compare_ignore_fields:
